...
const Flights = (props) => {
  ...
  const unloadProducts = async (depo_cat, id) => {
    try {
      const response = await axios.post(`${process.env.REACT_APP_BACK_URL}/flights/unload/${id}/${depo_cat}`)
  ...
  function makeFlightTable() {
    return flights.map((flight) => (
      <tr key={flight._id}>
        <td className="border border-slate-700  bg-slate-700 ">{flight._id}</td>
